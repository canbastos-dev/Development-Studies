...
uses uCliente, uVeiculo, ulocacao, uDTOLocacao, uDTOVeiculo, uDTOCliente,
  ...
  uCasoUsoCliente, uResponse, system.SysUtils;
  ...
  if idveiculo >  0 then begin
    ...
    if response.success then begin
      if locacao.Veiculo.id <>  locacao.VeiculoAtual.id then begin
        ...
        (responseVeiculo.Message = RetornaMsgResponse.ALTERADO_COM_SUCESSO) then
          result  := 'erro ao atualizar status do veículo'
        else begin
...
begin
  ...
  if response.success then result:=response.Message
  else result:='erro ao consultar';
...
begin
  ...
  if (response.success) and (response.Message = RetornaMsgResponse.CONSULTA_REALIZADA_SEM_RETORNO) then
  begin
    result  :=  'id locação invádilo';
    exit;
  end;

  response  := CasoUsoLocacao.Deletar(idlocacao);

  if response.success then result:= 'deletado com sucesso'
  else result:= 'erro ao deletar';
end;
